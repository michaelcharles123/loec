...
function add() {
    ...
    if (Matricula.value.toString() == '' || Matricula.value.length < 7 || Matricula.value.toString().indexOf(" ") >= 1) {
        ...
        if (!inLista(codigo.value.toUpperCase(), valores)) {
            ...
            lista = lista + '<li id="' + codigo.value.toString().substring(3, 12) + '" onclick="remove(this)">';

            lista += '<span class="barra" style="margin-right: 10px; ';

            lista += 'text-align: left; page-break-before:always;">';
...
var conteudoIframe = meuIframe.contentDocument;
var conteudoIframe = meuIframe.contentWindow.document;
 
meuIframe.contentWindow.document;
